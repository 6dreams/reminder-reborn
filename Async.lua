	...
	for config, queue in next, streams do
		for i, threadData in ipairs(queue) do
			...
			if not threadData.paused then
				...
				while (debugprofilestop() - start < max(1, maxExecutionTime)) do
					...
					if co and coroutine_status(co) ~= "dead" then
						...
						threadData.executionTime = threadData.executionTime + debugprofilestop() - s
						...
						if coroutine_status(co) == "dead" then -- function returned or errored
							...
							threadData:Kill()
							break -- we need a break to restart ipairs loop
					...
					if InCombatLockdown() and IsInInstance() and (debugprofilestop() - globalStart > 100) then
						return
					end
				end
			end
