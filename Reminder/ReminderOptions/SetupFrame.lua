...
function module.options:SetupFrameInitialize()
	...
	if not MRT.isClassic then
        ...
        for i=1,#prio do
            local diff = prio[i]
            local name = module.datas.instance_difficulty_types[diff]
            diffsList[#diffsList + 1] = {diff,name}
        end

        for diff,name in next, module.datas.instance_difficulty_types do
...
---------------------------------------
	...
	SetupFrame.sound = ELib:DropDown(SetupFrame.generalScrollFrame.C,270,15):Size(270):Point("TOP",SetupFrame.voiceCountdown,"BOTTOM",0,-5)
	do
	...
	SetupFrame.sound.testButton = MLib:Button(SetupFrame.sound):Size(20,20):Point("LEFT",SetupFrame.sound,"RIGHT",5,0):Tooltip("Play Sound"):OnClick(function()
		if SetupFrame.data.sound then
    ...
    SetupFrame.soundOnHide = ELib:DropDown(SetupFrame.generalScrollFrame.C,270,15):Size(270):Point("TOP",SetupFrame.sound,"BOTTOM",0,-5)
	do
		local function soundList_SetValue(_,arg1)
	...
	SetupFrame.soundOnHide.testButton = MLib:Button(SetupFrame.soundOnHide):Size(20,20):Point("LEFT",SetupFrame.soundOnHide,"RIGHT",5,0):Tooltip("Play Sound"):OnClick(function()
		if SetupFrame.data.soundOnHide then
            if VMRT.Reminder.disableSound then
                prettyPrint("Sound is disabled")
            else
	...
	SetupFrame.tts = MLib:MultiEdit(SetupFrame.generalScrollFrame.C,20,80):FontSize(14):Size(270,70):HideScrollOnNoScroll():Tooltip(
...
File names are case insensitive but extra spaces may ruin the file name match]]
	):Point("TOP",SetupFrame.soundOnHide,"BOTTOM",0,-5):OnChange(function(self,isUser)
		if isUser then
			local text, c = self:GetText():gsub("\n","")
            if c > 0 then
                self:SetText(text)
	...
	SetupFrame.tts.testButton = MLib:Button(SetupFrame.tts):Size(20,20):Point("TOPLEFT",SetupFrame.tts,"TOPRIGHT",5,0):Tooltip("Play TTS"):OnClick(function()
        if VMRT.Reminder.disableSound then
            prettyPrint("Sound is disabled")
        else
	...
	SetupFrame.glow = MLib:MultiEdit(SetupFrame.generalScrollFrame.C,20,80):FontSize(14):Size(270,70):HideScrollOnNoScroll():Point("TOP",SetupFrame.tts,"BOTTOM",0,-20):Tooltip(LR["Player names to glow\nMay use many separated by\nspace comma or semicolomn"]):OnChange(function(self,isUser)
		if isUser then
			local text, c = self:GetText():gsub("\n","")
            if c > 0 then
                self:SetText(text)
	...
	SetupFrame.glowFrameColor.preview.Update = function(self)
		...
		if bt then
            ...
            local color = VMRT.Reminder.Glow.Color
			at,rt,gt,bt = color:match("(..)(..)(..)(..)")
            if bt then
                local r,g,b,a = tonumber(rt,16),tonumber(gt,16),tonumber(bt,16),tonumber(at,16)
                self:SetColorTexture(r/255,g/255,b/255,a/255)
	...
	SetupFrame.glowFrameColor.colorButton:SetScript("OnClick",function(self)
        ...
        local colorPalette = SetupFrame.data.glowFrameColor or VMRT.Reminder.Glow.Color
        local at,rt,gt,bt = colorPalette:match("(..)(..)(..)(..)")
...
Or Atlas prefixed with 'A:' e.g. A:GarrMission_MissionIcon-Combat]]
            ...
            function button.spellID.dropDown:PreUpdate()
                ...
                while true do
                    ...
                    if isValid then
                        ...
                        tinsert(spellIDs,"List for difficulty: "..(module.datas.instance_difficulty_types[diff] or diff))
                        break
                    end
                end
	...
	do
        ...
        function SetupFrame.bossList:PreUpdate()
            ...
            if VMRT.Reminder.lastEncounterID then
				local JID = AddonDB.EJ_DATA.encounterIDtoEJ[VMRT.Reminder.lastEncounterID]
				local bossImg
				if JID and EJ_GetCreatureInfo then
					bossImg = select(5, EJ_GetCreatureInfo(1, JID))
				end
                tinsert(List,2,{
            ...
            for i=1,#encList do
                ...
                for j=#instance,2,-1 do
                    ...
                    local JID = AddonDB.EJ_DATA.encounterIDtoEJ[encounterID]
                    local bossImg
                    if JID and EJ_GetCreatureInfo then
                        bossImg = select(5, EJ_GetCreatureInfo(1, JID))
                    end
    ...
    do
		local function diffList_SetValue(_,diff)
            ...
            if SetupFrame.data.diff then
                local diff_name = module.datas.instance_difficulty_types[SetupFrame.data.diff]
	...
	SetupFrame.diffListRaw = ELib:Edit(SetupFrame.loadScrollFrame.C,nil,true):Size(270,20):Point("TOPRIGHT",SetupFrame.bossList,"BOTTOMRIGHT",0,-5):Tooltip(function() local diff = select(3,GetInstanceInfo()) return LR["Current difficulty:"] .. " " .. (diff or "none") end):OnChange(function(self,isUser)
        ...
        if text then
            local diff_name = module.datas.instance_difficulty_types[tonumber(text)]
    ...
    if not module.PUBLIC and AddonDB.RGAPI then
        ...
        SetupFrame.RGAPICondition = ELib:Edit(SetupFrame.loadScrollFrame.C):Size(240,20):Point("TOP",SetupFrame.RGAPIList,"BOTTOM",0,-5):Tooltip(LR.RGConditionsTip):OnChange(function(self,isUser)
            if isUser then
                local text = self:GetText()
                if text == "" then
                    text = nil
            ...
            if data.RGAPIList then
                ...
                if isOkay and type(list) == 'table' then
                    ...
                    if not passList then
                        list = "wrong condition"
                    else
                        list = table.concat(list," ")
                        list = list:gsub("([%S]+)",function(name)
                            local isInList = tContains(passList,name)
                ...
                SetupFrame.RGAPIPlayersCurr:SetText(isOkay and list or "---")
            else
    ...
    SetupFrame.personalChecks.disabled = ELib:Check(SetupFrame.tab.tabs[4],LR.PersonalDisable):Point("TOPLEFT",SetupFrame.comment,"BOTTOMLEFT",0,-5):OnClick(function(self)
        VMRT.Reminder.disabled[SetupFrame.data.token] = not VMRT.Reminder.disabled[SetupFrame.data.token] or nil
        SetupFrame:Update()
    end)
    SetupFrame.personalChecks.defEnabled = ELib:Check(SetupFrame.tab.tabs[4],LR.PersonalDisable):Point("TOPLEFT",SetupFrame.comment,"BOTTOMLEFT",0,-5):OnClick(function(self)
        VMRT.Reminder.defEnabled[SetupFrame.data.token] = not VMRT.Reminder.defEnabled[SetupFrame.data.token] or nil
        SetupFrame:Update()
    end)
    SetupFrame.personalChecks.locked = ELib:Check(SetupFrame.tab.tabs[4],LR.UpdatesDisable):Point("TOPLEFT",SetupFrame.personalChecks.defEnabled,"BOTTOMLEFT",0,-5):OnClick(function(self)
        VMRT.Reminder.locked[SetupFrame.data.token] = not VMRT.Reminder.locked[SetupFrame.data.token] or nil
        SetupFrame:Update()
    end)
    SetupFrame.personalChecks.disableSounds = ELib:Check(SetupFrame.tab.tabs[4],LR.SoundDisable):Point("TOPLEFT",SetupFrame.personalChecks.locked,"BOTTOMLEFT",0,-5):OnClick(function(self)
        VMRT.Reminder.disableSounds[SetupFrame.data.token] = not VMRT.Reminder.disableSounds[SetupFrame.data.token] or nil
        SetupFrame:Update()
    end)
    SetupFrame.personalChecks.lockedSounds = ELib:Check(SetupFrame.tab.tabs[4],LR.SoundUpdatesDisable):Point("TOPLEFT",SetupFrame.personalChecks.disableSounds,"BOTTOMLEFT",0,-5):OnClick(function(self)
        VMRT.Reminder.lockedSounds[SetupFrame.data.token] = not VMRT.Reminder.lockedSounds[SetupFrame.data.token] or nil
        SetupFrame:Update()
    end)
...
---------------------------------------
	...
	function SetupFrame:Update(UpdateHistory)
        ...
        if data.token then
            SetupFrame.personalChecks.disabled:SetChecked(VMRT.Reminder.disabled[data.token])
            SetupFrame.personalChecks.defEnabled:SetChecked(not VMRT.Reminder.defEnabled[data.token])
            SetupFrame.personalChecks.locked:SetChecked(VMRT.Reminder.locked[data.token])
            SetupFrame.personalChecks.disableSounds:SetChecked(VMRT.Reminder.disableSounds[data.token])
            SetupFrame.personalChecks.lockedSounds:SetChecked(VMRT.Reminder.lockedSounds[data.token])
        end
