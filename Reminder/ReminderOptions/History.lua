...
function module:SendHistory(historyEntry, archive_entry)
    ...
    AddonDB:SendComm("REM_HIST",str,nil,nil,nil,nil,nil,commsOptions)

    prettyPrint("History data sent",format("%.2f",debugprofilestop() - start), "ms in", ceil(#str/250), "parts")
end
...
function module:ProcessHistoryTextToData(sender, str, fromChat)
    ...
    local header, encoded = data:match("^(.-)##H##(.+)$")
    ...
    if not decoded then
        prettyPrint("Failed to decode history")
        return
    end

    local decompressed = LibDeflateAsync:DecompressDeflate(decoded)
    if not decompressed then
        prettyPrint("Failed to decompress history")
        return
    end

    local success, log = LibSerializeAsync:Deserialize(decompressed)
    if not success then
        prettyPrint("Failed to deserialize history")
        return
    end
    ...
    if not fromChat then
        if historyEntry.isMPlus then
            prettyPrint("Imported history for", GetRealZoneText(-encounterID), diff)
        else
            prettyPrint("Imported history for", LR.boss_name[encounterID], module.datas.instance_difficulty_types[diff] or diff)
        end
    end
...
function module:GetHistoryExportString(historyEntry,forChat,encoded)
    ...
    local serialized = LibSerializeAsync:SerializeEx(configForLS, log)
...
function module.options:InitHistory()
    ...
    local function GetEstimatedChanges(index,data)
        ...
        elseif index == 4 then
            if data[4] and data.timeFromStart then
                ...
                changes.ALL.delayTime = format("%01d:%04.1f", minutes, seconds)
            end
        elseif index == 5 then
        ...
        elseif index == 6 then
            if data[6] then
                if data.timeFromPrev then
                    ...
                    changes.SHIFT.delayTime = format("%01d:%04.1f", minutes, seconds)
                ...
                if data.timeFromPrevGlobal then
                    ...
                    changes.NORMAL.delayTime = format("%01d:%04.1f", minutes, seconds)
    ...
    function QuickList:HoverMultitableListValue(isEnter,index,obj)
        if not isEnter then
            ...
            if diff then
                GameTooltip:AddLine(LR["Any Click:"],1,1,1)
                GameTooltip:AddLine(LR["Difficulty"] ..ARROW ..(module.datas.instance_difficulty_types[diff] or ""))
            end
            ...
            if changes.NPC_INFO then
                ...
                if mobID then
                    ...
                    if C_TooltipInfo.GetHyperlink("unit:"..data.sourceGUID) then
                        local tooltipInfo = C_TooltipInfo.GetHyperlink("unit:"..data.sourceGUID)
                        local name = tooltipInfo and tooltipInfo.lines and tooltipInfo.lines[1] and tooltipInfo.lines[1].leftText
                        if name then
                            GameTooltip:AddDoubleLine(LR["Name"], name)
                        end
                    end
    ...
    local function FormatName(name,flags,GUID)
        ...
        elseif name and flags then
            ...
            elseif GUID and C_TooltipInfo.GetHyperlink(("unit:%s"):format(GUID)) then
                local tooltipInfo = C_TooltipInfo.GetHyperlink(("unit:%s"):format(GUID))
                if tooltipInfo and tooltipInfo.lines then
                    name = tooltipInfo.lines[1].leftText
                end
            end
    ...
    local function ButtonOnEnter(self)
        ...
        if currTable.isMPlus then
            encounterName = GetRealZoneText(-currTable.encounterID) or tostring(currTable.encounterID)
        else
            encounterName = LR.boss_name[currTable.encounterID]:gsub(",.+","")
        end
        GameTooltip:AddLine(MRT.F.utf8sub(encounterName,1, 30))
        GameTooltip:AddDoubleLine(LR["Fight timer"], FormatTime(currTable.duration), 1, 1, 1, 1, 1, 1)
        GameTooltip:AddDoubleLine(LR["Fight started"], date("%H:%M:%S %d/%m/%y", currTable.date), 1, 1, 1, 1, 1, 1)
        GameTooltip:AddDoubleLine(LR["Difficulty"], not currTable.isMPlus and module.datas.instance_difficulty_types[currTable.diff] or currTable.diff, 1, 1, 1, 1, 1, 1)
    ...
    function QuickList:UpdateSelectPullButtons()
        ...
        for i=1, #htable do
            ...
            local buttonText = (currTable.pinned and lockIcon or "") .. MRT.F.utf8sub(encounterName,1,28) .. " - #" .. i .. " " .. FormatTime(currTable.duration)
    ...
    function QuickList.HistoryDropDown:PreUpdate()
        ...
        for encounterID,tbl in next, module.db.history do
            ...
            if isZone then
                for zoneID,history in next, tbl do
                    local zoneName = GetRealZoneText(-zoneID) or zoneID
                    subMenu[#subMenu+1] = {
                        text = zoneName,
                        func = SetHistory,
                        arg1 = history,
        ...
        sort(List, function(a,b)
            ...
            if aIsNum and bIsNum then
                return module.GetEncounterSortIndex(a.encounterID,100000-a.encounterID) < module.GetEncounterSortIndex(b.encounterID,100000-b.encounterID)
            elseif aIsNum then
                return true
            elseif bIsNum then
                return false
    ...
    local function updateHistory(fromTrigger)
        ...
        for i=1,#tempHistory do
            ...
            if not searchPat then
                ...
                if spellName and spellName:lower():find(searchPat,1,true) then
                    searchPass = true
                else
                    for j=2,#line do
                        if line[j] and tostring(line[j]):lower():find(searchPat,1,true) then
                            searchPass = true
                            break
                        end
                    end
                ...
                then
                    ...
                    then
                        ...
                        result[#result+1] = {
                            ...
                            FormatTime(timestamp-startTime), -- time from start
                            phaseNow and "["..phaseNow.."] "..FormatTime(timestamp-phaseTime), -- time from phase
                            prevNowGlobal and FormatTime(timestamp-prevNowGlobal), -- time from previous
                    ...
                    then -- phase
                        result[#result+1] = {
                            ...
                            FormatTime(line[1]-startTime),
                            prevPhaseNow and "["..prevPhaseNow.."] "..FormatTime(line[1]-prevPhaseTime),
                            event = eventType,
                            rep = phaseRepeat,
                    ...
                    then -- chat msg
                        ...
                        result[#result+1] = {
                            ...
                            FormatTime(line[1]-startTime),
                            phaseNow and "["..phaseNow.."] "..FormatTime(line[1]-phaseTime),
                            nil, -- timeFromPrev?
                    ...
                    elseif eventType == 3 then -- enc start
                        ...
                        local diffName = module.datas.instance_difficulty_types[difficultyID]
                        result[#result+1] = {
                            encounterName,
                            diffName,
                            encounterID=encounterID,
