...
function module.options:InitializeInspect()
     ...
     inspectFrame.resetButton = MLib:Button(inspectFrame,"Reset"):Size(100,20):Point("RIGHT",inspectFrame.requestButton,"LEFT",-5,0):OnClick(function(self)
         if inspectFrame.selected then
             if IsInGroup() then
                 ...
                 module.db.inspectData[MRT.F.delUnitNameServer(inspectFrame.selected)] = nil
             end
     ...
     function inspectFrame.scrollList:ButtonClick(button) -- level 2 click
         ...
         if button == "RightButton" then
			MenuUtil.CreateContextMenu(self, function(ownerRegion,rootDescription, id)
				...
				if not MRT.F.UnitInGuild(inspectFrame.selected) then
					b1:SetEnabled(false)
					b1:SetTooltip(function(tooltip,elementDescription)
						tooltip:AddLine(MenuUtil.GetElementText(elementDescription))
						tooltip:AddLine(LR["Player has to be in the same guild to request WA"],1,1,1,true)
					end)
				end
     ...
     function inspectFrame:UpdateData()
        ...
        inspectFrame.Name:SetText(inspectFrame.selected)
        inspectFrame.Name:Color(MRT.F.classColorNum(UnitClassBase(inspectFrame.selected) or UnitClassBase(MRT.F.delUnitNameServer(inspectFrame.selected))))
...
function module:RequestWAMap(reciever, parent, maxDepth)
    parent = parent and parent or ""
    local isPass, reason = AddonDB:CheckSelfPermissions(WASync.isDebugMode)
    if isPass then
        MRT.F.SendExMsg("WASIR", MRT.F.CreateAddonMsg(reciever, parent, maxDepth))
    else
...
function module:SendWAMap(reciever,parent,maxDepth)
    module:Async(function()
        ...
        local serialized = LibSerializeAsync:SerializeEx(configForLS,map)
        local compressed = LibDeflateAsync:CompressDeflate(serialized,configForDeflate)
        local encoded = LibDeflateAsync:EncodeForWoWAddonChannel(compressed)

        local header = WASync.WAMAP_VERSION .. "^" .. reciever .. "##H##"
        AddonDB:SendComm("WAS_INS", header..encoded)
    end,"RequestWAMap", true)
end

AddonDB:RegisterComm("WAS_INS",function(prefix,sender,data)
    local header,body = data:match("^(.-)##H##(.+)$")
    local version, reciever = strsplit("^",header)
    ...
    if reciever and reciever ~= "" and Ambiguate(reciever,"none") ~= UnitName('player') then
        return
    end

    local decoded = LibDeflateAsync:DecodeForWoWAddonChannel(body)
    if not decoded then prettyPrint(WASYNC_ERROR, "Error decoding map") return end

    local decompressed = LibDeflateAsync:DecompressDeflate(decoded)
    if not decompressed then prettyPrint(WASYNC_ERROR, "Error decompressing map") return end

    local success, map = LibSerializeAsync:Deserialize(decompressed)
    if not success then prettyPrint(WASYNC_ERROR, "Erorr deserializing map") return end

    local updatedMap = module.db.inspectData[sender] or {}
    MergeWAMap(updatedMap, map)
    module.db.inspectData[sender] = updatedMap
