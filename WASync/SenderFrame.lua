...
function module:CreateSenderFrame()
    ...
    SenderFrame.updateConfigDropDown = ELib:DropDown(SenderFrame,230,10):Size(300,20):Point("BOTTOMLEFT",SenderFrame,"BOTTOMLEFT",10,110)
...
Always(F) - unchecks load never on import]])
    ...
    function SenderFrame.customTargetDropDown:PreUpdate()
        ...
        for index, name, subgroup, class, guid, rank, level, online in MRT.F.IterateRoster do
            if online then
                ...
                if MRT.F.UnitInGuild(name) then -- same guild limitation is set by MRT comms for WHISPER channel
                    local nick = AddonDB.RGAPI and AddonDB.RGAPI:UnitName(name) or name
                    list[key] = {
                        text = nick .. (" (whisper)"),
                        arg1 = "WHISPER",
                        arg2 = name,
                        colorCode =  "|c".. MRT.F.classColor(class),
                        func = customTargetDropDown_SetValue,
                        prio = 5,
                    }
                else
	...
	SenderFrame.SendButton = MLib:Button(SenderFrame,LR.ListdSend,15):Tooltip(LR["Pressing while holding |cff00ff00shift|r will add WA to queue but wont start sending\n\nPressing while holding |cff00ff00alt|r will not update last sync time for current WA(ignoring checkbox)\n\nPressing while holding |cff00ff00ctrl|r will start sending WAs added to queue"]):Size(380,30):Point("BOTTOM",SenderFrame,"BOTTOM",0,10):OnClick(function()
		if IsControlKeyDown() and module.SendingQueueFrame then
            for i=1,#SenderFrame.SendQueueFrame.queue do
                if SenderFrame.SendQueueFrame.queue[i].justEnqueued then
                    SenderFrame.SendQueueFrame.queue[i].justEnqueued = nil
            ...
            module.SendingQueueFrame:ProcessAll()
        else
            local id = SenderFrameData.id
            if id then
    ...
    function SenderFrame:InitBar(entryKey,id,total)
        -- SenderFrame.EstimateTimeLeft:SetText("Sending...")
    end
    ...
    function SenderFrame:UpdateBar(entryKey,current,total)
        ...
        SenderFrame.SendingProgressText:SetText(current * 251 .. "|cFF00FF00/" .. total * 251)
        ...
        -- SenderFrame.EstimateTimeLeft:SetText("Estimated time left: "..floor(max(#messageQueue*module.delayC,(total - current) * max(module.delayC,0.05))).." sec")
    end
