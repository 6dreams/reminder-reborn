...
QueueFrame.ImportModeText = ELib:Text(QueueFrame,""):Point("TOPLEFT",10,-86):Color()
...
QueueFrame.processNextButton = MLib:Button(QueueFrame,LR.Import,15):Point("BOTTOM",QueueFrame,"BOTTOM",0,30):Size(440,25):OnClick(function()
    QueueFrame:Hide()
    StaticPopup_Hide("WA_SYNC_SKIP_IMPORT_CONFIRMATION")
...
QueueFrame.skipNextButton = MLib:Button(QueueFrame,LR.Skip,15):Point("TOP",QueueFrame.processNextButton,"BOTTOM",0,-5):Size(440,20):OnClick(function()
    StaticPopupDialogs["WA_SYNC_SKIP_IMPORT_CONFIRMATION"] = {
        text = LR.WASyncUpdateSkipTitle,
        button1 = format("|cffff0000%s|r",LR.Skip),
        button2 = NO,
        OnAccept = function()
    ...
    StaticPopup_Show("WA_SYNC_SKIP_IMPORT_CONFIRMATION")
end)
...
function QueueFrame:Update()
    ...
    elseif QueueFrame.SessionStarted and not QueueFrame.IsImporting then
        ...
        QueueFrame.ImportModeText:SetText("Mode: " .. WASync.ImportTypes[queueItem.importMode])
...
function module.QueueFrame:AddToQueue(queueItem) -- dataStr, sender, id, importMode, stringNum, imageNum, skipPrompt
	if not queueItem.str or not queueItem.sender or not queueItem.id or not queueItem.importMode then
        return
    end
...
function module.QueueFrame:ProcessNext()
    ...
    module:ImportAsync(function()
        module:ImportWA(queueItem.str, queueItem.sender, queueItem.importMode ) --dataStr, sender, importMode
    end,"ImportWA")
end
...
function module:GetDiagonsticsForQueueItem(queueItem)
    ...
    local importMode = queueItem.importMode
    ...
    str = str .. "importMode (type: " .. type(importMode) .. "): " .. tostring(importMode) .. "\n"
